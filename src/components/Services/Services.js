export default function Services({
  ...
  const services = servicesList && servicesList.length > 0 ? servicesList : defaultServices;
    ...
    <section className="py-24 bg-gray-50 text-gray-900 relative">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <h2 
            className="text-4xl md:text-5xl font-extrabold tracking-tight mb-4 text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-blue-600 drop-shadow-sm"
            dangerouslySetInnerHTML={{ __html: title }}
          ></h2>
          {description && (
            <div 
              className="text-lg text-gray-600 max-w-2xl mx-auto [&>p]:mb-2"
          ...
          {services.map((service, idx) => (
            <div key={idx} className="group p-8 bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] hover:shadow-[0_10px_40px_rgba(0,0,0,0.1)] transition-all duration-300 transform hover:-translate-y-2 border border-gray-100 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="text-5xl mb-6 transform transition-transform group-hover:scale-110 group-hover:-rotate-3 inline-block">
