...
export default function Navbar({ menuItems = [], headerLogo = null }) {
  ...
  // Fallback links in case WordPress fetch fails or is empty
  const defaultLinks = [
    { id: 1, title: 'Home', url: '/' },
  ...
  const displayLinks = menuItems.length > 0 ? menuItems : defaultLinks;
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-gray-950/80 backdrop-blur-lg shadow-lg py-4 border-b border-white/10' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        {/* Logo */}
        <Link href="/" className={`text-2xl font-black tracking-tighter ${textColor} transition-colors flex items-center`}>
          {headerLogo ? (
            <img src={headerLogo} alt="Header Logo" className="h-10 w-auto object-contain" />
          ) : (
            <>
        ...
        <div className="hidden md:flex items-center gap-8">
          {displayLinks.map((link) => (
            <Link key={link.id} href={link.url} className={`text-sm font-medium transition-colors ${linkColor}`}>
              {link.title}
            </Link>
          ))}
        </div>
